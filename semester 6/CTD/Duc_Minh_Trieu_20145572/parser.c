...
Type* compileFactor(void) {
  ...
  switch (lookAhead->tokenType) {
    ...
    switch (obj->kind) {
     ...
     // neu la tham chieu thi phai load gia tri cua tham chieu len roi moi thuc hien duoc phep tinh
    ...
    if (isPredefinedFunction(obj)) {
     ...
     // tang T len 4  
     genINT(4);
     // sinh ma cho param // o trong expression
     compileArguments(obj->funcAttrs->paramList);
        // giam T di k+4
...
Type* compileIndexes(Type* arrayType) {
  ...
  while (lookAhead->tokenType == SB_LSEL) {
    eat(SB_LSEL);
    type = compileExpression();
    // compileExpression se dat gia tri index len dinh stack
    checkIntType(type);
