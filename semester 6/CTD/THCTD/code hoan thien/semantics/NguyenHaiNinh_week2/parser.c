...
void compileProgram(void) {
	...
	enterBlock(program->proAttrs->scope);
...
ConstantValue* compileUnsignedConstant(void) {
	...
	case TK_NUMBER:
		eat(TK_NUMBER);
		constValue = makeIntConstant(currentToken->value); //add code here
		break;
	case TK_IDENT:
		eat(TK_IDENT);
		//add code here
	...
	case TK_CHAR:
		eat(TK_CHAR);
		constValue = makeCharConstant(currentToken->string[0]);//add code here
		break;
	default:
